...
define('ADMIN_PASSWORD','ran@123!');      // Admin Password - CHANGE THIS TO ENABLE!!!
...
EOB;
    ...
    if(!empty($_SERVER['APP_SERVER_NAME']))
        echo "<tr class=tr-0><td class=td-0>APCu Host</td><td>{$_SERVER['APP_SERVER_NAME']} $host</td></tr>\n";
...
EOB;
    ...
    foreach($cache[$scope_list[$MYREQUEST['SCOPE']]] as $i => $entry) {
        if ($entry['creation_time'] < 1000000000) {
            $entry['creation_time'] += $cache['start_time'];
            $entry['access_time'] += $cache['start_time'];
            $entry['mtime'] += $cache['start_time'];
        }
